...
=======================================================================================
Usage:
  python process_video.py --video cctv_test.mp4                         # YOLO26n default
  python process_video.py --video cctv_test.mp4 --skip 2               # faster
  python process_video.py --video cctv_test.mp4 --model yolo11          # use YOLO11n
  python process_video.py --video cctv_test.mp4 --imgsz 640 --skip 3   # RPi5
  python process_video.py --video cctv_test.mp4 --no-show               # headless
"""
...
def process(video_in,
    ...
    print("=" * 65)
    print("CCTV TRACKING  model=%s  conf=%.2f  imgsz=%d  skip=%d" % (
        model_label, conf, imgsz, frame_skip))
...
if __name__ == "__main__":
    ...
    p.add_argument("--imgsz",    type=int,   default=960,
                   help="YOLO input size: 960=default, 640=RPi5, 1280=max (needs RAM)")
    p.add_argument("--skip",     type=int,   default=1,
                   help="Frame skip: 1=every frame, 2=2x faster, 3=RPi5 recommended")
    p.add_argument("--model",    default=None,
                   help="Model to use: yolo26 (default, NMS-free) or yolo11")
    p.add_argument("--no-show",  action="store_true",
                   help="Disable display window (use on RPi5 over SSH)")
    a = p.parse_args()
    process(a.video, a.output, not a.no_show, a.conf, a.skip, a.imgsz, a.model)
